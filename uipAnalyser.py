...
def uip_analysis(packet_list):      #Função de análise UDP/IP
    ...
    try:
        ...
        print('[*] %s is not on the source list' %target_ip)        #Falha ao remover o IP alvo da lista de IP fonte
    try:
        connection_dst.remove(target_ip)        #Tenta remover o IP alvo da lista de IP destino
    except:
        print('[*] %s is not on the destination list' %target_ip)       #Falha ao remover o IP alvo da lista de IP destino
    x = len(connection_dst)     #Seta a variável X com o tamanho da lista de IP destino
    y = len(connection_src)     #Seta a variável Y com o tamanho da lista de IP fonte
    ...
    for i in range(len(connection_array)):
        ...
        for j in range(len(packet_list)):
            if packet_list[j].haslayer(UDP):        #Verifica se o pacote tem cabeçalho UDP
                if packet_list[j][IP].src == connection_array[i]:       #Verifica se o IP é um uploader
                    flagsrc += 1
                elif packet_list[j][IP].dst == connection_array[i]:     #Verifica se o IP é um downloader
                    flagdst += 1
        print('-------------------------------------------------------------------')
        print('[*] The IP %s uploaded %i and downloaded %i packets!!!' %(connection_array[i], flagsrc, flagdst))    #Mostra a quantidade de downloads/uploads do IP
    ...
    for i in range(len(mostly_updloader)):
        print('=> %s' %mostly_updloader[i])     #Mostra todos os uploaders
    print('-------------------------------------------------------------------') 